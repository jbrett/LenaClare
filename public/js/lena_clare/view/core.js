define(function (require) {
    ...
    function animate_nivo($progress, speed) {
        $progress.find('span').animate({
                        'width': '100%'
        }, speed, 'linear');
    }
    
    function reset_nivo($progress) {
        $progress.find('span').stop().css('width', '0%');
    }
    
    function resize_menu(width) {
        if(width > 979) {
    ...
    return {
        init: init,
        animate_nivo: animate_nivo,
        reset_nivo: reset_nivo,
